...
const Week27 = () => {
  ...
  const processData = (text) => {
    ...
    }).filter(item => !/^\d+$/.test(item.attackerName));
    ...
    return (
      <div className="team-with-patronage">
        {heroes.map((hero, index) => {
          // Первый герой (Аль) отображается без покровителя
          if (index === 0) {
            return (
          ...
          // Для остальных героев берем покровителя из patronagePairs по порядку
          const patronPair = patronagePairs[index - 1]; // -1 потому что первый герой без покровителя
